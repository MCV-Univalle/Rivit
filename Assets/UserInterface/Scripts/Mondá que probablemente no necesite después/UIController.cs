...
public abstract class UIController : MonoBehaviour
{
    ...
    protected bool _skipScoreCountingActive = false;
    ...
    public void Update()
    {
        if(Input.GetMouseButtonDown(0) && _skipScoreCountingActive)
        {
            _skipScoreCountingActive = false;
            _finalScore.text = "" + _gameManager.Score;
        }
    ...
    public virtual IEnumerator ShowResultsPanel()
    {
        ...
        StartCoroutine(IncrementFinalScore());
        DestroyRanking();
    ...
    public virtual IEnumerator IncrementFinalScore() //The score displayed in the results panel
    {
        _skipScoreCountingActive = true;
        int newHighScore = VerifyNewHighScore();
        StartCoroutine(FadeElement(_scorePanel, true));
        yield return new WaitForSeconds(0.35F);
        for(int i = 0; (i <= _gameManager.Score) && _skipScoreCountingActive; i++)
        {
            _finalScore.text = "" + i;
