	...
	public class ClassDrawer : UnityObjectDrawer<ClassDrawer, Object>, IRootDrawer
	{
		...
		private static bool DoesGameObjectHaveReferencesToType(GameObject gameObject, Type targetType)
		{
			...
			for(int c = components.Length - 1; c >= 0; c--)
			{
				...
				var fields = componentType.GetFields(ParentDrawerUtility.BindingFlagsDeclaredInstance);
				...
				var properties = componentType.GetProperties(ParentDrawerUtility.BindingFlagsDeclaredInstance);
		...
		{
			ParentDrawerUtility.GetStaticMemberBuildList(this, MemberHierarchy, ref memberBuildList, DebugMode, PropertyVisibility.AllPublic, MethodVisibility.AllPublic);
		}
