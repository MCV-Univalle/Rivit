...
namespace Sisus
{
	...
	public static class ParentDrawerUtility
	{
		...
		//DeclaredOnly makes it so we ignore inherited ones which is useful when manually looping through base types
		public const BindingFlags BindingFlagsDeclaredInstance = BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly;
		public const BindingFlags BindingFlagsDeclaredStatic = BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.DeclaredOnly;
		public const BindingFlags BindingFlagsDeclaredInstanceAndStatic = BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly;
		...
		{
			var result = GetNextInternalSelectableFieldUpInternal(subject, column, requester, membersListedVertically);
		...
		/// <param name="membersListedVertically"> True to members listed vertically. </param>
		/// <returns> The next selectable drawer up. This may be null. </returns>
		private static IDrawer GetNextInternalSelectableFieldUpInternal(IParentDrawer subject, int column, IDrawer requester, bool membersListedVertically)
		{
		...
		{
			var result = GetNextInternalSelectableFieldDownInternal(subject, column, requester, membersListedVertically);
		...
		/// <param name="membersListedVertically"> True to members listed vertically. </param>
		/// <returns> The next selectable drawer up. This may be null. </returns>
		private static IDrawer GetNextInternalSelectableFieldDownInternal([NotNull]IParentDrawer subject, int column, [CanBeNull]IDrawer requester, bool membersListedVertically)
		{
			var members = subject.VisibleMembers;
			int count = members.Length;
			...
			if(count > 0)
			{
				if(membersListedVertically)
				{
					...
					{
						#if DEV_MODE && DEBUG_NEXT_FIELD
						Debug.Log("\""+subject.Name+"\" - "+subject.GetType().Name + ".GetNextSelectableDrawerDown("+column+"): members[0] (because requester is self)");
						#endif
					...
					if(index == -1)
					{
						...
						{
							#if DEV_MODE && DEBUG_NEXT_FIELD
							Debug.Log("\""+subject.Name+"\" - "+subject.GetType().Name + ".GetNextSelectableDrawerDown("+column+"): members[0] (because requester not member and subject not selectable)");
							#endif
							return members[0].GetNextSelectableDrawerDown(0, subject);
						}

						#if DEV_MODE && DEBUG_NEXT_FIELD
						Debug.Log("\""+subject.Name+"\" - "+subject.GetType().Name + ".GetNextSelectableDrawerDown("+column+"): self (because requester not member)");
						#endif
						return subject;
					}
					...
					{
						#if DEV_MODE && DEBUG_NEXT_FIELD
						Debug.Log("\""+subject.Name+"\" - "+subject.GetType().Name + ".GetNextSelectableDrawerDown("+column+"): members["+ nextIndex + "] (because nextIndex < count)");
						#endif
				...
				else //members listed horizontally
				{
					...
					{
						#if DEV_MODE && DEBUG_NEXT_FIELD
						Debug.Log("\""+subject.Name+"\" - "+subject.GetType().Name + ".GetNextSelectableDrawerDown("+column+"): parent (because requester is self)");
						#endif
					...
					if(index == -1)
					{
						...
						{
							#if DEV_MODE && DEBUG_NEXT_FIELD
							Debug.Log("\""+subject.Name+"\" - "+subject.GetType().Name + ".GetNextSelectableDrawerDown("+column+ "): self (because column <= 1)");
							#endif
						...
						#if DEV_MODE && DEBUG_NEXT_FIELD
						Debug.Log("\""+subject.Name+"\" - "+subject.GetType().Name + ".GetNextSelectableDrawerDown(" + column + "): members[" + nextIndex + "] (because column > 0)");
						#endif
			...
			{
				#if DEV_MODE && DEBUG_NEXT_FIELD
				Debug.Log("\"" + subject.Name + "\" - " + subject.GetType().Name + ".GetNextSelectableDrawerDown(" + column + "): self (because no members and requester not self)");
				#endif
			...
			{
				#if DEV_MODE && DEBUG_NEXT_FIELD
				Debug.Log("\""+subject.Name+"\" - "+subject.GetType().Name + ".GetNextSelectableDrawerDown("+column+"): parent (because index >= count)");
				#endif
				return subject.Parent.GetNextSelectableDrawerDown(column, subject);
			}
			
			#if DEV_MODE && DEBUG_NEXT_FIELD
			Debug.Log("\""+subject.Name+"\" - "+subject.GetType().Name + ".GetNextSelectableDrawerDown("+column+"): FocusedDrawer (because index >= count && Parent null)");
			#endif
		...
		public static void GetMemberBuildList([NotNull]IUnityObjectDrawer parent, [NotNull]LinkedMemberHierarchy hierarchy, [NotNull]ref List<LinkedMemberInfo> results, [NotNull]DebugModeDisplaySettings debugModeSettings, [CanBeNull]string[] skipMembers)
		{
			GetMemberBuildList(parent, hierarchy, ref results, debugModeSettings);
		...
		public static void GetMemberBuildList([NotNull]IUnityObjectDrawer parent, [NotNull]LinkedMemberHierarchy hierarchy, [NotNull]ref List<LinkedMemberInfo> results, [NotNull]DebugModeDisplaySettings debugModeSettings)
		{
			...
			if(debugModeSettings.Static)
			{
				...
				bindingFlags = BindingFlagsDeclaredStatic;
			}
			else
			{
				parentType = LinkedMemberParent.UnityObject;
				bindingFlags = BindingFlagsDeclaredInstance;
			}

			if(debugModeSettings.ShowFields)
			{
				for(var type = topLevelType; type != null; type = type.BaseType)
				{
		...
		public static void GetMemberBuildList([NotNull]IParentDrawer parent, [NotNull]LinkedMemberHierarchy hierarchy, [NotNull]ref List<LinkedMemberInfo> results, bool includeHidden, BindingFlags bindingFlags = BindingFlagsDeclaredInstanceAndStatic)
		{
			#if DEV_MODE && PI_ASSERTATIONS
		...
		public static void GetMemberBuildList([NotNull]IParentDrawer parent, [NotNull]LinkedMemberHierarchy hierarchy, [NotNull]ref List<LinkedMemberInfo> results, DebugModeDisplaySettings debugModeSettings, BindingFlags bindingFlags = BindingFlagsDeclaredInstanceAndStatic)
		{
			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(bindingFlags.HasFlag(BindingFlags.Instance) || bindingFlags.HasFlag(BindingFlags.Static), "GetMemberBuildList called with neither instance nor static flags enabled. Nothing will be returned.");
			Debug.Assert(bindingFlags.HasFlag(BindingFlags.Public) || bindingFlags.HasFlag(BindingFlags.NonPublic), "GetMemberBuildList called with neither public nor nonpublic flags enabled. Nothing will be returned.");
			...
			{
				parentType = topLevelType.IsUnityObject() ? LinkedMemberParent.UnityObject : LinkedMemberParent.Missing;
			}
			else
			{
		...
		public static void GetMemberBuildList([NotNull]IParentDrawer parent, [NotNull]LinkedMemberHierarchy hierarchy, [NotNull]ref List<LinkedMemberInfo> results, bool includeHidden, FieldVisibility showNonSerializedFields, PropertyVisibility propertyVisibility, MethodVisibility methodVisibility, BindingFlags bindingFlags = BindingFlagsDeclaredInstance)
		{
			...
			Debug.Assert((bindingFlags & BindingFlags.DeclaredOnly) != 0, "GetMemberBuildList was called with bindingFlags that wasn't DeclaredOnly");
			#endif

			var fieldInfo = parent.MemberInfo;
			var topLevelType = parent.Type;
			...
			for(var type = topLevelType; type != null; type = type.BaseType)
			{
				...
				//DeclaredOnly makes it so we ignore inherited ones
				//which is what we want since we are manually looping through all base types
				//(and manually stopping when we'ver reached the depth we want to)
				type.GetInspectorViewables(ref hierarchy, fieldInfo, ref results, bindingFlags, includeHidden, showNonSerializedFields, propertyVisibility, methodVisibility);
			}
		}
		
		public static void GetStaticMemberBuildList([NotNull]IParentDrawer parent, [NotNull]LinkedMemberHierarchy hierarchy, [NotNull]ref List<LinkedMemberInfo> results, bool includeHidden, PropertyVisibility propertyVisibility, MethodVisibility methodVisibility)
		{
			var fieldInfo = parent.MemberInfo;
			var topLevelType = parent.Type;
			...
			for(var type = topLevelType; type != null; type = type.BaseType)
			{
				...
				//which is what we want since we are manually looping through all base types
				//(and manually stopping when we'ver reached the depth we want to)
				type.GetStaticInspectorViewables(ref hierarchy, fieldInfo, ref results, includeHidden, propertyVisibility, methodVisibility);
			}
		}
				...
				switch(order[t])
				{
					...
					case Member.Property:
						for(; n < count; n++)
						{
							...
							var member = drawerProvider.GetForProperty(linkedInfo.CanRead ? linkedInfo.GetValue(0) : linkedInfo.Type.DefaultValue(), linkedInfo.Type, linkedInfo, parent, label, readOnly);
		...
		private static bool HandleGroupAttributeForField([NotNull]IDrawerProvider drawerProvider, [CanBeNull]IParentDrawer parent, LinkedMemberInfo linkedInfo, List<IDrawer> results, ref Dictionary<string, ICustomGroupDrawer> groupDrawers, ref Dictionary<string, List<IDrawer>> membersByGroup)
		{
			...
			{
				groupDrawer = groupAttribute.CreateDrawer(parent, readOnly);
