...
namespace Sisus
{
	...
	{
		private const int SelectNoneMenuItemIndex = 0;
		...
		private static void FindMenuItemsForFlag(List<PopupMenuItem> searchItems, Enum targetFlag, ref List<PopupMenuItem> results, Enum noneValue)
		{
			for(int n = searchItems.Count - 1; n >= 0; n--)
			{
				...
				if(item.IsGroup)
				{
					FindMenuItemsForFlag(item.children, targetFlag, ref results, noneValue);
				}
				else
				{
