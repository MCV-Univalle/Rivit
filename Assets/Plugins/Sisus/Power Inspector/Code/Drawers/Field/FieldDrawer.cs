...
//#define DEBUG_NULL_VALUE_AND_FIELDINFO
...
namespace Sisus
{
	...
	public abstract class FieldDrawer<TValue> : BaseDrawer, IFieldDrawer
	{
		...
		private bool CanWriteToField
		{
			get
			{
				return canWriteToField && !ReadOnly;
			}
		}
		...
		protected virtual void Setup([CanBeNull]TValue setValue, [NotNull]Type setValueType, [CanBeNull]LinkedMemberInfo setMemberInfo, [CanBeNull]IParentDrawer setParent, [CanBeNull]GUIContent setLabel, bool setReadOnly)
		{
			...
			else if(setMemberInfo != null)
			{
				...
				// New test: allow readonly controls to be edited in the inspector if they have a parent field which is not readonly.
				// E.g. KeyValuePair members are readonly properties, but should still be editable, with the changes being applied on the parent level.
				else if(setParent != null && setParent.MemberInfo != null && setParent.MemberInfo.CanWrite)
				{
					readOnly = false;
					canWriteToField = false;
				}
			...
			{
				if(setMemberInfo != null) { Debug.Log(StringUtils.ToColorizedString(ToString(), " readOnly=", readOnly, ", setReadOnly=", setReadOnly, ", canWriteToField=", canWriteToField, ", setMemberInfo=", setMemberInfo, ", setMemberInfo.CanWrite=", setMemberInfo.CanWrite, ", setParent.MemberInfo=", setParent == null ? null : setParent.MemberInfo)); }
				else { Debug.Log(StringUtils.ToColorizedString(ToString(), " readOnly=", readOnly, ", setReadOnly=", setReadOnly, ", canWriteToField=", canWriteToField, ", setMemberInfo=", null, ", setParent.MemberInfo=", setParent == null ? null : setParent.MemberInfo)); }
			}
			#endif
		...
		public override void CopyToClipboard()
		{
			...
			Clipboard.TryCopy(Value, Value == null ? Type : Value.GetType());
			SendCopyToClipboardMessage();
		}
		...
		protected override void DoPasteFromClipboard()
		{
			...
			if(type.IsAbstract || type == Types.SystemObject)
			{
		...
		/// <summary>
		/// Determine if we can paste from clipboard.
		/// </summary>
		/// <returns>
		/// True if we can paste from clipboard, false if not.
		/// </returns>
