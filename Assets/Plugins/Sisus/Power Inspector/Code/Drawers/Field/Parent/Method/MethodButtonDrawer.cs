	...
	public class MethodButtonDrawer : MethodDrawer
	{
		...
		protected override void GetDrawPositions(Rect position)
		{
			...
			if(label.text.Length == 0)
			{
				...
				if(!DrawInSingleRow)
				{
					const float foldoutWidth = 12f;
					buttonRect.x = labelLastDrawPosition.x + foldoutWidth;
					buttonRect.width = lastDrawPosition.width - foldoutWidth - (labelLastDrawPosition.x - lastDrawPosition.x) - DrawGUI.RightPadding;
					labelLastDrawPosition.width = foldoutWidth;
				}
				else
				{
