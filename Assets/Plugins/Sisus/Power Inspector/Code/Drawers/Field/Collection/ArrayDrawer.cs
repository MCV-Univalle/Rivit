...
namespace Sisus
{
	public class ArrayDrawer : OneDimensionalCollectionDrawer<Array>
	{
		...
		protected override object GetCollectionValue(Array collection, int index)
		{
			...
			{
				Debug.LogError(ToString()+ ".GetCollectionValue called for null collection!\n"+e);
			#else
		...
		private static object GetCollectionSizeStatic(object collection)
		{
			...
			{
				Debug.LogError("ArrayDrawer.GetCollectionSizeStatic called for null array!\n"+e);
			#else
			catch(NullReferenceException)
			{
			#endif
		...
		private static void SetArraySizeStatic(ref object collection, object size)
		{
