...
namespace Sisus
{
	...
	public struct Size3D : IEquatable<Size3D>, IEquatable<int>
	{
		...
		public Size3D(int setHeight, int setWidth, int setDepth)
		{
			...
			#if DEV_MODE && PI_ASSERTATIONS
			if(setHeight < 0 && !Equals(Invalid))
			{
				UnityEngine.Debug.LogWarning("Size3D contructor called with unexpected parameters: "+setHeight+"x"+setWidth+"x"+setDepth);
			}
			else if(setWidth < 0 && !Equals(Invalid))
			{
				UnityEngine.Debug.LogWarning("Size3D contructor called with unexpected parameters: "+setHeight+"x"+setWidth+"x"+setDepth);
			}
			else if(setDepth < 0 && !Equals(Invalid))
			{
				UnityEngine.Debug.LogWarning("Size3D contructor called with unexpected parameters: "+setHeight+"x"+setWidth+"x"+setDepth);
			}
			#endif
