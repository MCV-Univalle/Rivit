...
namespace Sisus
{
	...
	[Serializable]
	public sealed class PropertyDrawerDrawer : PrefixControlComboDrawer<object>, IPropertyDrawerDrawer, ITextFieldDrawer
	{
		private UnityEditor.PropertyDrawer drawerInstance;
		private bool editField;
		private bool usesFoldout;
		...
		[CanBeNull]
		public static PropertyDrawerDrawer Create(object value, Type drawerType, LinkedMemberInfo memberInfo, IParentDrawer parent, GUIContent label, bool readOnly)
		{
			if(memberInfo == null)
			{
				#if DEV_MODE
			...
			result.Setup(value, drawerInstance, drawerType, memberInfo, parent, label, readOnly);
			result.LateSetup();
			return result;
		}
		...
		public void SetupInterface(object attribute, object setValue, LinkedMemberInfo setMemberInfo, IParentDrawer setParent, GUIContent setLabel, bool setReadOnly)
		{
			...
			if(attribute != null)
			{
				if(!CustomEditorUtility.TryGetPropertyDrawerType(attribute.GetType(), out drawerType))
				{
					#if DEV_MODE
			...
			{
				if(setMemberInfo == null || setMemberInfo.Type.IsAbstract || !CustomEditorUtility.TryGetPropertyDrawerType(setMemberInfo.Type, out drawerType))
				{
					if(setValue == null || !CustomEditorUtility.TryGetPropertyDrawerType(setValue.GetType(), out drawerType))
					{
						drawerType = null;
						#if DEV_MODE
						if(setValue == null)
			...
			Setup(setValue, null, drawerType, setMemberInfo, setParent, setLabel, setReadOnly);
		}
