...
namespace Sisus
{
	...
	public abstract class CustomEditorBaseDrawer<TSelf, TTarget> : UnityObjectDrawer<TSelf, TTarget>, ICustomEditorDrawer where TSelf : CustomEditorBaseDrawer<TSelf, TTarget> where TTarget : Object
	{
		...
		/// <summary> Updates the main custom Editor used for drawing the body. </summary>
		protected virtual void UpdateEditor()
		{
			if(canHaveEditor)
			{
				...
				Editors.GetEditor(ref editor, targets, editorType, allSameType);
		...
		public override bool Draw(Rect position)
		{
			...
			if(Platform.EditorMode)
			{
				...
				if(MembersAreVisible)
				{
					...
					float leftIndent = DrawGUI.indentWidth * LeftIndent;
					pos.x += leftIndent;
		...
		private bool DrawMembers(Rect position)
		{
			bool dirty = false;
			...
			GUILayout.BeginHorizontal();
			{
				...
				var style = bodyEditor != null && (!bodyEditor.UseDefaultMargins()) ? GUIStyle.none : EditorStyles.inspectorDefaultMargins;
			...
			//making sure GUILayout.GetLastRect works
			//even if OnInspectorGUI had GUILayout.Space at the end
			GUILayoutEmpty();
		...
		private void OnInspectorGUI(Editor bodyEditor)
		{
			...
			// NOTE: It's important NOT to wrap this inside a try-catch statement
			// ExitGUIExceptions will happen and interrupt the code here if they are used.
			if(bodyEditor != null)
			{
				if(ReadOnly)
				{
		...
		protected override void DoGenerateMemberBuildList()
		{
			if(!UsesEditorForDrawingBody)
			{
				...
				ParentDrawerUtility.GetMemberBuildList(this, linkedMemberHierarchy, ref memberBuildList, debugModeDisplaySettings, NeverDisplayMembers());
			}
		}
		...
		/// <summary> Get list of members members that should not be displayed in Debug mode nor during filtering. </summary>
		/// <returns> A string array containg names of members that should not be shown. If null, all members can be shown in debug mode and when filtering. </returns>
		[CanBeNull]
		protected virtual string[] NeverDisplayMembers()
		{
			return null;
		}
