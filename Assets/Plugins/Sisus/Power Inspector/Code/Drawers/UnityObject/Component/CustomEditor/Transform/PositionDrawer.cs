...
namespace Sisus
{
	...
	public class PositionDrawer : TransformMemberBaseDrawer
	{
		...
		public void Ground()
		{
			...
			bool changed = false;
			var values = GetValues();
			bool localSpace = Inspector.State.usingLocalSpace;
			...
			for(int n = unityObjects.Length - 1; n >= 0; n--)
			{
				...
				if(transform != null)
				{
					...
					if(hit.HasValue)
					{
						...
						var setWorldPosition = transform.position;
						...
						if(collider != null)
						{
							...
							setWorldPosition.y += bounds.extents.y;
						}
						else
						{
							...
							if(meshFilter != null)
							{
								...
								if(mesh != null)
								{
									...
									setWorldPosition.y += bounds.extents.y * transform.lossyScale.y;
								}
							}
						}

						var setValue = localSpace ? transform.InverseTransformPoint(setWorldPosition) : setWorldPosition;
						if(!ValuesAreEqual((Vector3)values[n], setValue))
						{
							values[n] = localSpace ? transform.InverseTransformPoint(setWorldPosition) : setWorldPosition;
							changed = true;
						}						
					}
				}
