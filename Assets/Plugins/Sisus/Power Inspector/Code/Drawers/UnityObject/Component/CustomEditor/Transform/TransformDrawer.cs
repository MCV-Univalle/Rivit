...
namespace Sisus
{
	...
	public class TransformDrawer : ComponentDrawer, ITransformDrawer
	{
		...
		private Rect ToggleCoordinateSpaceIconPosition
		{
			get
			{
				var rect = LeftmostPrefixIconPosition;
				rect.x -= ComponentHeaderToolbarIconWidth + ComponentHeaderToolbarIconsOffset;
				rect.width = ComponentHeaderToolbarIconWidth;
				return rect;
			}
		}

		private Rect ToggleSnapToGridIconPosition
		{
			get
			{
				var rect = ToggleCoordinateSpaceIconPosition;
				rect.x -= ComponentHeaderToolbarIconWidth + ComponentHeaderToolbarIconsOffset;
				return rect;
			}
		}

		private Rect Toggle2DModeIconPosition
		{
			get
			{
				var rect = LeftmostPrefixIconPosition;
		...
		/// <inheritdoc />
		protected override void UpdateHeaderToolbarPositions(Rect headerRect)
		{
			base.UpdateHeaderToolbarPositions(headerRect);
			for(int n = headerParts.Count - 1; n >= 0; n--)
			{
				var part = headerParts[n];
				switch((TransformHeaderPart)part.Part)
				{
					case TransformHeaderPart.ToggleLocalSpaceButton:
						part.Rect = ToggleCoordinateSpaceIconPosition;
						break;
					case TransformHeaderPart.ToggleSnapToGridButton:
						part.Rect = ToggleSnapToGridIconPosition;
						break;
				}
			}
		}

		private void OnToggleLocalSpaceIconClicked(Event inputEvent)
		{
			DrawGUI.Use(inputEvent);
			SetUsingLocalSpace(!UsingLocalSpace);
			GUI.changed = true;
			SelectHeaderPart((HeaderPart)TransformHeaderPart.ToggleLocalSpaceButton);
		}
