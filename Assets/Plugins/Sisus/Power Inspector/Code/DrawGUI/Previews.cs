...
namespace Sisus
{
	...
	public class Previews
	{
		...
		{
			previewablesByTarget = new Dictionary<Type, List<Type>>(100);

			#if UNITY_EDITOR
			var ipreviewableInterface = Types.Editor.GetInterface("UnityEditor.IPreviewable");
			var ipreviewables = ipreviewableInterface.GetImplementingTypes(true);
			...
			for(int n = ipreviewables.Length - 1; n >= 0; n--)
			{
				var previewableType = ipreviewables[n];
			...
			#if DEV_MODE
			//UnityEngine.Debug.Log("previewablesByTarget.Count: "+previewablesByTarget.Count);
			#endif
		}
