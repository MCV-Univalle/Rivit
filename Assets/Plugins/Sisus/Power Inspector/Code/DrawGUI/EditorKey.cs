	...
	public struct EditorKey : IEquatable<EditorKey>
	{
		...
		public EditorKey([NotNull]Object[] targets, bool isAssetImporterEditor)
		{
			...
			unchecked // Overflow is fine, just wrap
			{
				...
				#if DEV_MODE && PI_ASSERTATIONS
				else { UnityEngine.Debug.Assert(!(targets[0] is UnityEditor.AssetImporter), StringUtils.TypeToString(targets[0])+ " was an AssetImporter but isAssetImporterEditor was "+StringUtils.False); }
				#endif
			}
		}
