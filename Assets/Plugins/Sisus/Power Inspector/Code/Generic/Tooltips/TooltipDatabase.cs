...
namespace Sisus
{
	...
	public class TooltipDatabase
	{
		...
		/// </summary>
		private readonly Dictionary<Type, Dictionary<string, string>> memberTooltipsByClass = new Dictionary<Type, Dictionary<string, string>>(5000);
		
		/// <summary>
		/// Cached tooltips for members sorted by ICustomAttributeProvider (i.e. MemberInfo or ParameterInfo).
		/// </summary>
		private readonly Dictionary<ICustomAttributeProvider, string> memberTooltips = new Dictionary<ICustomAttributeProvider, string>(5000);
			...
			switch(rootAssembly)
			{
				...
				default:
					...
					for(int n = guids.Length - 1; n >= 0; n--)
					{
						...
						string scriptAssetFullPath = FileUtility.LocalToFullPath(scriptAssetLocalPath);
						if(TryFetchMemberTooltips(scriptAssetFullPath, classType, out tooltips))
						{
							memberTooltipsByClass.Add(classType, tooltips);
							return tooltips;
						}
						...
						#if DEV_MODE
						Debug.LogWarning("TryFetchMemberTooltips(\""+scriptAssetFullPath+"\", "+(classType == null ? "null" : classType.FullName) + ") failed.");
						#endif
					}
		...
		public Dictionary<string, string> FetchMemberTooltips([NotNull]MonoScript monoScript)
		{
			...
			string scriptAssetFullPath = FileUtility.LocalToFullPath(scriptAssetLocalPath);
			return FetchMemberTooltips(scriptAssetFullPath, monoScript.GetClass());
		}
		#endif

		[NotNull]
		private Dictionary<string, string> FetchMemberTooltips(string scriptAssetFullPath, [CanBeNull]Type classTypeMustMatch)
		{
			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(scriptAssetFullPath.IndexOf(':') != -1);
			Debug.Assert(scriptAssetFullPath.EndsWith(".cs", StringComparison.OrdinalIgnoreCase));
			#endif

			var tooltips = new Dictionary<string, string>();
			ScriptAssetDocumentationCommentParser.ParseComments(scriptAssetFullPath, tooltips, classTypeMustMatch);
			return tooltips;
		}

		private bool TryFetchMemberTooltips(string scriptAssetFullPath, [CanBeNull]Type classTypeMustMatch, [NotNull]out Dictionary<string, string> tooltips)
		{
			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(scriptAssetFullPath.IndexOf(':') != -1);
			Debug.Assert(scriptAssetFullPath.EndsWith(".cs", StringComparison.OrdinalIgnoreCase));
			...
			return ScriptAssetDocumentationCommentParser.ParseComments(scriptAssetFullPath, tooltips, classTypeMustMatch);
		}
		...
		public string GetTooltip([NotNull]LinkedMemberInfo linkedInfo)
		{
			...
			#if DEV_MODE && DEBUG_GET_TOOLTIP && UNITY_EDITOR
			Debug.Log(linkedInfo+ " no tooltip found.");
			#endif
