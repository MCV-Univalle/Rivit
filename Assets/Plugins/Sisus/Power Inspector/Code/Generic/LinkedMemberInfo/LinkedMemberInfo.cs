	...
	public class LinkedMemberInfo
	{
		...
		public string DisplayName
		{
			get
			{
				if(displayName == null)
				{
					displayName = StringUtils.MakeFieldNameHumanReadable(Name);
				}
				return displayName;
			}
		}
		...
		public void SetupCollectionMember([NotNull]LinkedMemberInfo setParent, Type type, int collectionIndex, GetCollectionMember getDelegate, SetCollectionMember setDelegate, string serializedPropertyRelativePath = null)
		{
			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(setParent != null);
			#endif
			...
			if(TryGetSerializedProperty(serializedPropertyRelativePath, out serializedProperty, null))
			{
		...
		public void Setup([CanBeNull]LinkedMemberInfo setParent, [NotNull]FieldInfo fieldInfo, LinkedMemberParent setParentType, string serializedPropertyRelativePath = null)
		{
			#if DEV_MODE && PI_ASSERTATIONS
			...
			#if UNITY_EDITOR
			SerializedProperty serializedProperty;
			if(TryGetSerializedProperty(serializedPropertyRelativePath, out serializedProperty, fieldInfo))
			{
				#if DEV_MODE && PI_ASSERTATIONS
				Debug.Assert(serializedProperty != null, serializedPropertyRelativePath);
				#endif
		...
		public void Setup([CanBeNull]LinkedMemberInfo setParent, [NotNull]PropertyInfo propertyInfo, LinkedMemberParent setParentType, string serializedPropertyRelativePath = null)
		{
			parent = setParent;
			parentType = setParentType;

			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(memberData != null, "LinkedMemberInfo Setup called with memberData being null!\nparent=" + StringUtils.ToString(setParent)+", propertyInfo="+propertyInfo+", serializedPropertyPath="+StringUtils.ToString(serializedPropertyRelativePath));
			...
			Debug.Assert(serializedPropertyRelativePath == null || (propertyInfo != null && !propertyInfo.IsStatic()), "static property had serializedPropertyRelativePath="+StringUtils.ToString(serializedPropertyRelativePath));
			#endif
			...
			#if UNITY_EDITOR
			SerializedProperty serializedProperty;
			if(TryGetSerializedProperty(serializedPropertyRelativePath, out serializedProperty, propertyInfo))
			{
		...
		public void SetupIndexer(LinkedMemberInfo setParent, [NotNull]PropertyInfo propertyInfo, LinkedMemberParent setParentType, string serializedPropertyRelativePath = null)
		{
			parent = setParent;
			parentType = setParentType;
		...
		public void Dispose()
		{
			...
			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(memberData != null, GetType().Name+" (\""+ displayName + "\") Dispose called with memberData "+StringUtils.Null);
			#endif
			...
			{
				Debug.LogError(GetType().Name + " (\"" + displayName + "\") memberData.Dispose called with memberData="+(memberData == null ? StringUtils.Null : "NotNull")+" "+e);
			}
