...
namespace Sisus
{
	...
	public static class ContextMenuUtility
	{
		...
		private static void OpenContextMenu()
		{
			...
			if(openingMenuPosition.HasValue)
			{
				throw new NotImplementedException("OpenContextMenu called @ "+openingMenuPosition.Value+" but not yet supported at runtime.");
			}
			throw new NotImplementedException("OpenContextMenu called but not yet supported at runtime.");
			#endif
		}
					...
					if(property.objectReferenceValue != null)
					{
						...
						if(manager != null)
						{
							...
							if(inspector != null)
							{
								...
								menu.AddItem(new GUIContent("Peek", "Open target in split view in Power Inspector."), false, ()=>splittableDrawer.ShowInSplitView(property.objectReferenceValue));
							}
						}
					}
				}
