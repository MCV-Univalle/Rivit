...
namespace Sisus
{
	public static class FileUtility
	{
		...
		public static string LocalToFullPath(string localPath, bool isPackage)
		{
			...
			return isPackage ? GetPackagePath(localPath) : LocalAssetsPathToFullPath(localPath);
			#else
		...
		#if UNITY_2017_2_OR_NEWER
		public static string GetPackagePath(string localPath)
		{
			#if UNITY_EDITOR
			...
			// get full path to local application data - e.g. "C:\Users\[UserName]\AppData\Local"
			string localAppDataPath = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);

			// get full path to unity's packages cache
			string packagesPath = string.Concat(localAppDataPath, @"\Unity\cache\packages\packages.unity.com\");
			
			var packagesType = Types.GetInternalEditorType("UnityEditor.PackageManager.Packages");
			if(packagesType == null)
			{
				#if DEV_MODE
				Debug.LogError("Failed to find UnityEditor.PackageManager.Packages type");
				#endif
				return "";
			}

			var getPackageInfoMethod = packagesType.GetMethod("GetForAssetPath");
			if(getPackageInfoMethod == null)
			{
				#if DEV_MODE
				Debug.LogError("Failed to find Packages.GetForAssetPath method");
				#endif
				return "";
			}

			var packageInfo = (UnityEditor.PackageManager.PackageInfo)getPackageInfoMethod.InvokeWithParameter(null, localPath);
			
			if(packageInfo == null)
			{
				#if DEV_MODE
				Debug.LogError("Failed to get PackagePath from localPath "+localPath);
				#endif
				return "";
			}

			// get full path to Unity Package directory containing the localPath
			string packagePath = string.Concat(packagesPath, packageInfo.packageId);
			
			// trim the beginning part away from the local path to get the local path
			// relative to the Unity Package path
			string packageRelativePath = localPath.Substring(9 + packageInfo.name.Length);
			
			//finally combine the Unity Package path and the Unity Package relative local path to get the full path to the file
			string fullPath = string.Concat(packagePath, packageRelativePath);
		...
		public static string LocalAssetsPathToFullPath(string localPath)
		{
			...
			Debug.Assert(localPath.StartsWith("Assets", StringComparison.OrdinalIgnoreCase), "LocalToFullPath: localPath " + StringUtils.ToString(localPath) + " did not start with \"Assets\"");
			#endif
