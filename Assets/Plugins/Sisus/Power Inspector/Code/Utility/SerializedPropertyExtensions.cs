	...
	public static class SerializedPropertyExtensions
	{
		public static void GetMemberInfoAndOwner(this SerializedProperty prop, out MemberInfo memberInfo, out object owner)
		{
			...
			for(int n = 0; n <= last; n++)
			{
				...
				if(fieldInfo != null)
				{
					Debug.Log("GetMemberInfoAndOwner(\"" + prop.propertyPath + "\") - pathPart #"+(n+1)+"/"+ (previousToLast + 2) +" \"" + pathPart+"\" fieldInfo found...");
				...
				else
				{
					...
					Debug.Log("GetMemberInfoAndOwner(\"" + prop.propertyPath + "\") - pathPart #" + (n + 1) + "/" + (previousToLast + 2) + " \"" + pathPart + "\" propertyInfo found...");
