#define SHOW_ALL_PROPERTIES_AS_FIELDS_WHEN_POSSIBLE
...
namespace Sisus
{
	public static class MemberInfoExtensions
	{
		...
		public static bool ShowInspectorViewableAsNormalField(this PropertyInfo property)
		{
			...
			//as normal fields.
			#if SHOW_ALL_PROPERTIES_AS_FIELDS_WHEN_POSSIBLE
			return getter.IsPublic; //UPDATE: if getter is public, we'll assume that it's safe to display as a normal field
			#else
			return false;
			#endif
		}
		...
		public static bool IsInspectorViewable(this MethodInfo method, MethodVisibility methodVisibility, bool includeHidden)
		{
			...
			{
				if(method.IsDefined(Types.ContextMenu, true))
				{
					var contextMenu = method.GetCustomAttributes(Types.ContextMenu, true)[0] as ContextMenu;
		...
		private static bool TryGetAttributeDeterminedInspectorVisibility(MemberInfo member, out bool showInInspector, bool skipNonSerialized = true)
		{
