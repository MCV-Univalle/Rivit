...
namespace Saltos
{
    public class UIManager : UIController
    {
        ...
        [SerializeField]
        TextMeshProUGUI contadorAyudasLineas;
        [SerializeField]
        GameObject botonAyudaLineas;
        [SerializeField]
        GameObject instruccionesPanel;
        [SerializeField]
        GameObject[] gifTurorial;
        int numGifVisible = 0;
        ...
        public void SetContAyudaLineas(int valor)
        {
            contadorAyudasLineas.text = valor.ToString();
        }

        public void SetVisibleBotonAyudaLineas(bool valor)
        {
            botonAyudaLineas.SetActive(valor);
        }


        public void SetVisiblePanelInstrucciones(bool valor)
        {
            instruccionesPanel.SetActive(valor);
        }

        public void ExecuteBotonSiguientePanelTurorial()
        {
            if (numGifVisible < gifTurorial.Length-1)
            {
                numGifVisible += 1;
                foreach (GameObject item in gifTurorial)
                {
                    item.SetActive(false);
                }
                gifTurorial[numGifVisible].SetActive(true);
            }
        }

        public void ExecuteBotonAtrasPanelTurorial()
        {
            if (numGifVisible > 0)
            {
                numGifVisible -= 1;
